      ...
      {items.map((item) => (
        <Card key={item.id} className="cursor-pointer hover:shadow-xl transition-shadow bg-white">
          ...
          <CardContent className="p-4 mt-10">
            <CardTitle className="text-gray-700 text-lg">{item.title}</CardTitle>
          ...
          <div className="bg-gray-100 p-4">
            <p className="text-sm text-gray-500 py-1">Content Date <span className="font-bold text-black-lg">{item.date}</span></p>
            <p className="text-sm text-gray-500 py-1">Content Type <span className="font-bold text-black-lg">{item.type}</span></p>
          </div>
        </Card>
      ))}
    </div>
